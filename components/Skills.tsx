            ...
            {courses.map((courseType, index) => {
              return courseType.classes.map((course) => {
                return (
                  <div
                    key={`course-${index}`}
                    className="flex items-center justify-center flex-row gap-2 capitalize text-xs font-semibold bg-white text-slate-700 rounded-3xl px-6 py-4 font-bol text-center"
                  >
                    {course}
                  </div>
                );
